...
const repositorySideTreeImplementation: ReplacementSelectorSet = {
	...
	filename: '.PRIVATE_TreeView-item-content > .PRIVATE_TreeView-item-content-text',
	icon: '.PRIVATE_TreeView-item-visual svg',
	isDirectory: (_rowEl, _fileNameEl, iconEl) => iconEl.getAttribute('class')!.includes('octicon-file-directory-'),
...
const repositoryMainImplementation: ReplacementSelectorSet = {
	...
	icon: 'svg',
	isDirectory: (_rowEl, _fileNameEl, iconEl) => iconEl.classList.contains('icon-directory'),
	isSubmodule: (_rowEl, fileNameEl, _iconEl) => fileNameEl.getAttribute('aria-label')!.includes('(Submodule)'),
...
const pullRequestTreeImplementation: ReplacementSelectorSet = {
	...
	isDirectory: (_rowEl, _fileNameEl, iconEl) => iconEl.getAttribute('aria-label') === 'Directory',
	isSubmodule: (_rowEl, _fileNameEl, iconEl) => iconEl.getAttribute('aria-label') === 'Submodule',
	isCollapsable: (rowEl, fileNameEl, iconEl) => pullRequestTreeImplementation.isDirectory(rowEl, fileNameEl, iconEl),
};
